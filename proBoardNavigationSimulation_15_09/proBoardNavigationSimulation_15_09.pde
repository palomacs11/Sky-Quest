...
color gray = color(72, 71, 71);
color red = color(250, 10, 10);
color pink = color(250, 10, 126);
color purple = color(149, 10, 250);
color blue = color(10, 16, 252);
color inter1 = lerpColor(red, pink, 0.60);
color inter2 = lerpColor(pink, purple, 0.50);
color inter3 = lerpColor(purple, blue, 0.60);
...
void draw() {
  ...
  printTemperaturePanel();  //--Print the temperature panel. 
...
//****NEED TO PROVIDE AN EXPLANATION OF WHAT I'M DOING HERE**//
void printTemperaturePanel() {
  pushMatrix();        //--Start of the object. 
  translate(950, 100);

  //--Map values read from temperature sensor and map it to print temperature pointer.
  float posTriangle1 = map( tempIMU, -44, 85, 60, 480); 
  float posTriangle2 = map(tempSens, 0, 50, 60, 480);

  //--Draw triangle pointers (temperature indicators)
  fill(orange);
  stroke(yellow);
  triangle(posTriangle1 - 10, tpY - 25, posTriangle1, tpY - 5, posTriangle1 + 10, tpY - 25);
  fill(orange);
  stroke(yellow);
  triangle(posTriangle2 - 10, tpY + 70, posTriangle2, tpY + 50, posTriangle2 + 10, tpY + 70);

  //--Draw the temperature panel.
  stroke(yellow);
  strokeWeight(2);
  fill(orange);
  textSize(16);
  text("-44°C", tpX, tpY + 10);
  text("  0°C", tpX, tpY + 40);
  fill(blue);
  rect(60, tpY, l, w);
  fill(inter3); 
  ...
  rect(420, tpY, l, w);
  fill(orange);
  text("85°C", 485, tpY + 10);
  text("50°C", 485, tpY + 40);
  popMatrix();
}
